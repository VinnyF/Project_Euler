...
int main() {
    ...
    //Put all the data in a 2-D vector
    int iter = 0;
    for (int i = 0; i < 20; i++) {
        std::vector<int> row;
        for (int j = 0; j < 20; j++) {
            row.push_back(grid_a[iter]);
            iter++;
        }
        grid.push_back(row);
    }
    ...
    for (unsigned int i = 0; i < grid.size(); i++) {
        for (unsigned int j = 0; j < grid[i].size()-4; j++) {
    ...
    //Check vertical
    for (unsigned int i = 0; i < grid.size()-4; i++) {
    ...
    for (unsigned int i = 3; i < grid.size()-4; i++) {
        for (unsigned int j = 3; j < grid[i].size()-4; j++) {
            long product = grid[i][j]*grid[i+1][j-1]*grid[i+2][j-2]*grid[i+3][j-3];
            if (product > answer) answer = product;
        }
    }
